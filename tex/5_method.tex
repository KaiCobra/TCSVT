...
\textit{(iii) The preserve level $\ell$ is not a tuned hyperparameter.} The preserve mask is consumed only by Phase~2, where it anchors background positions to source tokens while the target-side attention is recorded; the tokens generated in that pass are discarded (Sec.~\ref{sec:pipeline}). Consequently $\ell$ influences the final output only through whether the target attention peaks cross $\tau_k$, which it does not affect. We set $\ell = h - 0.01$ throughout and verify in Sec.~\ref{sec:beta} that the choice is inconsequential.
...
For the first $N$ coarse scales, all tokens are replaced with source tokens $r_k^s$ regardless of the masks, because these scales encode the low-frequency global layout. This anchoring is an explicit design choice with a measured alternative. A common implicit variant converts the per-scale binary masks into a cumulative probability field, i.e., replacing each position with a probability given by the average of all masks from scale $0$ to $k$; because coarse scales are almost fully replaced, this systematically inflates the replacement probability at fine scales and turns the composition into a biased stochastic anchoring. Replacing it with strictly binary composition plus an explicit $N$ gives uniformly better results, and $N$ itself trades background fidelity against layout freedom; we use $N=2$ throughout. Sec.~\ref{sec:anchoring} reports both comparisons.
