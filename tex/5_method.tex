...
The parameters $(h, c_{\max}, \kappa_{\max})$ are not hand-tuned; Sec.~\ref{sec:calibration} describes how they are obtained.
%% Values used in this paper: h = 0.50, c_min = 0, c_max = 0.33, kappa_min = 0,
%% kappa_max = 0.43, l = 0.49.  Stated in Sec. VI (implementation details).
...
Composition needs to know which regions the edit will affect before any token is replaced. Since the source-side mask alone cannot capture regions newly introduced by the target prompt, we perform a second generation pass conditioned on the target prompt $P_t$, anchoring positions where $M_{k,s}^{\mathrm{p}} = 1$ to source tokens $r_k^s$ to prevent background drift. From the resulting cross-attention, we extract the target focus mask $M_{k,t}^{\mathrm{f}}$ via the same filtering and thresholding procedure, and mask it against the preserve mask,
\begin{equation}
    M_{k,t}^{\mathrm{f}} \leftarrow M_{k,t}^{\mathrm{f}} \wedge \neg M_{k,s}^{\mathrm{p}},
    \label{eq:target_focus_gate}
\end{equation}
so that positions already identified as pure background are not pulled into the edited region by a residual target-side attention response.

We stress that \textbf{every token produced by this pass is discarded, and $M_{k,t}^{\mathrm{f}}$ is its only output} --- no token from Phase~2 reaches the final image. This is the structural fact on which property~(iii) of Sec.~\ref{sec:threshold} rests.
