...
scales, and anchoring those scales to the source is what keeps the background
stable (Sec.~\ref{sec:frontier}); the same anchoring necessarily
...
entirely is strictly worse than keeping it. What
...
difference contains words that are not edit-relevant. On this benchmark $230$
of the $700$ source prompts yield an empty focus set. A prompt encoder that
identifies edit-relevant spans directly, rather than by set difference, would
