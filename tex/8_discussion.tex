...
interpolated codes of every scale before accumulation. On the full benchmark this
...
\subsection{What Cannot Be Predicted Before Generation}

Sec.~\ref{sec:ceiling} establishes an upper bound and a negative result: a
per-case choice of threshold would improve every metric simultaneously, but that
choice correlates with no measurable property of the attention map
($|\rho| \le 0.23$ over nineteen factors), and prompt semantics predict only
whether an edit leans towards preservation or freedom, not where it breaks
($R^2 \approx 0$). Two directions remain open. One is to predict the entire
metric-versus-threshold curve rather than its maximum, and select an operating
point on the predicted curve --- this keeps the method single-shot. The other is
to observe a signal after generation has begun, when the information that is
missing beforehand becomes available. Both are outside the scope of this paper.
