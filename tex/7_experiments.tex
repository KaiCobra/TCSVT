...
Following prior work~\cite{PIEBench,AREdit}, we adopt six metrics grouped into two aspects. For \textit{background preservation}, we report Structure Distance~\cite{PIEBench} (S.D.), PSNR, SSIM, and LPIPS~\cite{LPIPS}. For \textit{editing quality}, we report CLIP similarity~\cite{CLIP} on both the whole image (CLIP whole) and the edited region (CLIP edited). We additionally report two preference models, HPSv2 and ImageReward, which we use throughout Sec.~\ref{sec:analysis}. All metrics are computed using the official PIE-Bench evaluation pipeline; Appendix~\ref{app:eval} documents the exact protocol, including the prompt convention used for scoring. Inference time per image is also reported.
% TODO(bib): add HPSv2 and ImageReward entries to main.bib and cite them here.
...
method attains the best SSIM ($0.8606$) and the best CLIP whole ($0.2658$) among
all methods, together with the second-best LPIPS ($0.0875$) and CLIP edited
($0.2332$). On ImageReward we reach $0.7098$, ahead of two of the four 12B
flow-based baselines and behind the other two, with $6\times$ fewer parameters.
Preservation and edit quality are usually traded against one another --- the
strongest preservation baselines sit near the bottom of the preference metrics
and vice versa --- so holding the leading position on the preservation side
while remaining in the leading group on the preference side is the substantive
result here, and it is obtained with a 2B backbone in $2.5$ seconds per image.

\textit{vs.\ diffusion-based methods.}
Compared to the best diffusion-based method PnP Inversion, we improve SSIM by $8.0\%$ and LPIPS by $17.5\%$ while achieving higher CLIP scores on both whole-image and edited-region metrics.

\textit{vs.\ flow-based methods.}
Against the strongest flow-based competitor ReFlex, we attain better SSIM
($0.8606$ vs.\ $0.8436$) and LPIPS ($0.0875$ vs.\ $0.0961$) despite the
$6\times$ parameter gap, while ReFlex achieves higher PSNR ($25.33$ vs.\
$22.53$) and ImageReward ($0.7334$ vs.\ $0.7098$). Across the four flow-based
baselines as a group we lead on both preservation metrics and sit third of five
on ImageReward, ahead of RF-Inversion ($0.5484$) and FireFlow ($-0.3778$).

\textit{vs.\ AREdit.}
AREdit~\cite{AREdit} is the closest comparison: a training-free editor on the same 2B VAR backbone. We obtain better SSIM ($0.8606$ vs.\ $0.8370$), CLIP whole ($0.2658$ vs.\ $0.2542$) and CLIP edited ($0.2332$ vs.\ $0.2277$), while AREdit is better on Structure Distance ($0.0305$ vs.\ $0.0421$) and PSNR ($24.19$ vs.\ $22.53$), the two methods being level on LPIPS ($0.0870$ vs.\ $0.0875$). The two methods therefore occupy neighbouring points on the same trade-off curve rather than one dominating the other, and the comparison should be read alongside two differences in what is required of the user: AREdit calibrates cached token statistics with per-category thresholds supplied externally, whereas our masks are derived from the model's own attention with a single parameterization shared by all ten categories; and the operating point itself is exposed as a continuous control in our method (Sec.~\ref{sec:frontier}), so the preservation end of our frontier can be selected explicitly when Structure Distance and PSNR are the quantities of interest.
...
% TODO(resolution): either re-run the baselines at a matched resolution or state
% the resolution caveat explicitly in the caption of Table I.
