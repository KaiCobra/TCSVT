...
We build upon Infinity~\cite{Infinity} (2B parameters) with BSQ-VAE~\cite{BSQVAE} ($d{=}32$). Generation uses $S{=}13$ scales ($1{\times}1$ to $64{\times}64$), CFG scale 4.0, and temperature $0.5$. Text conditioning uses Flan-T5-XL~\cite{flan-t5}. We set $N{=}2$ anchoring scales. For mask binarization we use the instance-adaptive threshold of Sec.~\ref{sec:threshold} with $h{=}0.50$, $c_{\min}{=}0$, $c_{\max}{=}0.20$, $\kappa_{\min}{=}0$, $\kappa_{\max}{=}0.60$ and $\ell = h - 0.01$; these values are derived by the protocol of Sec.~\ref{sec:protocol}. For IQR filtering, we discard transformer blocks whose attention deviation from the cross-block mean exceeds the IQR fence (Sec.~\ref{sec:mask_construction}), using blocks 2--31 of the 32-block model (blocks 0--1 are excluded as they consistently exhibit the global attention effect described in Sec.~\ref{sec:mask_construction}). All experiments run on a single NVIDIA RTX 5090 GPU with ${\sim}$2.5\,s inference per image.
% TODO(verify): confirm (h, c_max, kappa_max) against the CLI log of the result
% directory behind Table I before submission.

\subsection{Comparison with State-of-the-Art Methods}

\noindent\textbf{Quantitative results.}
Table~\ref{tab:method_comparison_wo_kvedit} summarizes the comparison. Our
method attains the best SSIM ($0.8565$), the best LPIPS ($0.0833$) and the best
ImageReward ($0.7344$) among all methods, together with the second-best CLIP
whole ($0.2613$) and CLIP edited ($0.2290$). Preservation and edit quality are
usually traded against one another --- the strongest preservation baselines sit
near the bottom of the preference metrics and vice versa --- so leading on both
sides simultaneously is the substantive result here, and it is obtained with
only 2B parameters, $6\times$ fewer than all flow-based baselines.
