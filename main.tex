...
%
% Marking convention (paper-wide): the colour gradient carries the ranking
% information (darker = better), and boldface is reserved exclusively for the
% row reporting our own method, so that the same configuration is recognisable
% across every table.  The fourth argument is therefore a plain bold flag:
% pass 1 on our row, 0 everywhere else.  No cell is bolded or underlined for
% merely being the column best.
% ---------------------------------------------------------------------------
...
\input{tex/2_introduction}   % I.    Introduction
\input{tex/3_relatedWorks}  % II.   Related Work
\input{tex/4_preliminaries} % III.  Preliminaries
\input{tex/5_method}        % IV.   Methodology
\input{tex/6_analysis}      % V.    What controls the preservation-edit trade-off
\input{tex/7_experiments}   % VI.   Experiments
\input{tex/8_discussion}    % VII.  Discussion and Limitations
\input{tex/9_conclusion}    % VIII. Conclusion
