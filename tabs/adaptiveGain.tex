...
%% an independent holdout (see tab:protocol).
...
%% Convention: bold = the parameters selected by the protocol; no per-column
%% ranking.  These are offline estimates, NOT our reported scores (Table VI).
\begin{table}[t]
\centering
...
arms). \textbf{Bold} marks the parameters selected by the protocol of
Sec.~\ref{sec:protocol}. All values here are offline estimates on the dev set
($n{=}560$) and are used for relative ranking only; the directly measured scores
of our method are those of Table~\ref{tab:method_comparison_wo_kvedit}.}
...
\rowcolor{gray!12}
$(c_{\max}, \kappa_{\max}) = (0.20, 0.60)$ (selected) & \textbf{0.734} & \textbf{0.856} & \textbf{0.0916} & \textbf{21.98} \\
...
selected @ SSIM $0.856$ & \multicolumn{4}{l}{$0.734$ vs.\ $\approx 0.704$ interpolated \quad $(+0.030)$}\\
selected @ LPIPS $0.0916$ & \multicolumn{4}{l}{$0.734$ vs.\ $\approx 0.702$ interpolated \quad $(+0.032)$}\\
